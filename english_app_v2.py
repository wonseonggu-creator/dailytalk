# -*- coding: utf-8 -*-
"""
DailyTalk 서버 v1.0 (Phase 1)
- 같은 폴더의 dailytalk.html을 그대로 서빙 (단일 소스 — py 안에 HTML 내장 없음)
- /api/progress : 기기 간 진행 동기화 (dailytalk_progress.json)
- /sw.js        : 오프라인용 서비스 워커
실행:  python english_app_v2.py   →  PC: http://localhost:8378
...
HTML_PATH = os.path.join(BASE, "dailytalk.html")
PROG_PATH = os.path.join(BASE, "dailytalk_progress.json")

SW_JS = """
const C='dailytalk-v1';
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path in ("/", "/index.html", "/dailytalk.html"):
            if not os.path.exists(HTML_PATH):
                self._hdr(500)
                self.wfile.write(
                    "dailytalk.html 파일이 없습니다. 서버와 같은 폴더에 두세요.".encode("utf-8")
                )
                return
    ...
    def do_POST(self):
        ...
        if path == "/api/check":
            try:
                ...
                out = ai_check(d.get("meaning", ""), d.get("target", ""), d.get("answer", ""), "Korean")
...
def main():
    if not os.path.exists(HTML_PATH):
        print("[!] dailytalk.html 이 같은 폴더에 없습니다. 함께 두고 다시 실행하세요.")
    srv = ThreadingHTTPServer(("0.0.0.0", PORT), H)
    ip = local_ip()
    print("=" * 46)
    print("  DailyTalk — 매일 10분 영어회화 (Phase 1)")
    print("=" * 46)
